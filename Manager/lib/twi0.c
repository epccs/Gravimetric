...
void twi0_init(uint8_t pull_up)
{
    ...
    ioDir(MCU_IO_SCL0, DIRECTION_INPUT); // DDRC &= ~(1 << DDC4)
    ioDir(MCU_IO_SDA0, DIRECTION_INPUT); // DDRC &= ~(1 << DDC5)
    ...
    if (pull_up) 
    {
        ioWrite(MCU_IO_SCL0, LOGIC_LEVEL_HIGH); // PORTC |= (1 << PORTC4) weak pullup
        ioWrite(MCU_IO_SDA0, LOGIC_LEVEL_HIGH); // PORTC |= (1 << PORTC5) weak pullup
    }
...
void twi0_disable(void)
{
    ...
    ioWrite(MCU_IO_SCL0, LOGIC_LEVEL_LOW); // PORTC &= ~(1 << PORTC4) disable the pull-up
    ioWrite(MCU_IO_SDA0, LOGIC_LEVEL_LOW); // PORTC &= ~(1 << PORTC5)
}
