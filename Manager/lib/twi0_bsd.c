...
uint8_t twi0_masterBlockingRead(uint8_t slave_address, uint8_t* read_data, uint8_t bytes_to_read, TWI0_PROTOCALL_t send_stop)
{
    ...
    else
    {
        ...
        while(status == TWI0_RD_STAT_BUSY)
        {
            status = twi0_masterAsyncRead_status();
        }

        // read can fail but this don't care.
        if ( (status == TWI0_RD_STAT_ADDR_NACK) || (status == TWI0_RD_STAT_DATA_NACK) || (status == TWI0_RD_STAT_ILLEGAL) )
            return 0;

        // read operation complete so get data
        uint8_t bytes_read = twi0_masterAsyncRead_getBytes(read_data);
        return bytes_read;
    }
}
